...
def images(G,device,inputs,space,truncation_psi,label,noise_mode,outdir,start=None,stop=None):
    ...
    for idx, i in enumerate(inputs):
        ...
        PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/frame{idx:06d}.png')
...
def truncation_traversal(G,device,z,label,start,stop,increment,noise_mode,outdir):
    ...
    while trunc <= stop:
        ...
        img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/frame{count:06d}.png')
...
def generate_images(
    ...
    if(process=='image'):
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            img = G(z, label, truncation_psi=truncation_psi, noise_mode=noise_mode)
            img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/seed{seed:06d}.png')
